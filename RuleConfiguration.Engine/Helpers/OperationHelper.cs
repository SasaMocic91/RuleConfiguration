...
using RuleConfiguration.Engine.Configuration;
...
public class OperationHelper : IOperationHelper
{
    ...
    public IOperation GetOperationByName(string operationName)
    {
        ...
        if (operation == null) throw new OperationNotFoundException(operationName);
    ...
    private HashSet<IOperation> GetSupportedOperations(Type type)
    {
        ...
        supportedOperations.AddRange(Operations.Where(o =>
            o.TypeGroup.HasFlag(typeGroup) && !o.SupportsLists && o.Active));

        if (underlyingNullableType != null)
            supportedOperations.AddRange(Operations.Where(o =>
                o.TypeGroup.HasFlag(TypeGroup.Nullable) && !o.SupportsLists && o.Active));
    ...
    {
        if (!overloadExisting) return;

        var op = _operations.FirstOrDefault(o =>
            string.Compare(o.Name, operationName, StringComparison.InvariantCultureIgnoreCase) == 0);
        if (op != null) op.Active = false;
    }
}