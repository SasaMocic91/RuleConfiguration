...
public class FilterBuilder
{
    ...
    private Expression ProcessListStatement(ParameterExpression param, IFilterStatement statement)
    {
        ...
        var propertyName = statement.PropertyId.Substring(statement.PropertyId.IndexOf("[") + 1)
    ...
    {
        var memberType = member.Member.MemberType == MemberTypes.Property
            ? ( member.Member as PropertyInfo )?.PropertyType
            : ( member.Member as FieldInfo )?.FieldType;
    ...
    {
        var parentName = memberName.Substring(0, memberName.IndexOf(".", StringComparison.InvariantCultureIgnoreCase));
